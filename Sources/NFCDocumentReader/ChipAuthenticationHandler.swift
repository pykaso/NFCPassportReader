...
class ChipAuthenticationHandler {
    ...
    public func doChipAuthentication() async throws  {
        ...
        guard isChipAuthenticationSupported else {
            throw NFCDocumentReaderError.NotYetSupported( "ChipAuthentication not supported" )
        }
        ...
        if !success {
            throw NFCDocumentReaderError.ChipAuthenticationFailed
        }
    }
    
    private func doChipAuthentication( with chipAuthPublicKeyInfo : ChipAuthenticationPublicKeyInfo ) async throws -> Bool {
        
        // So it turns out that some documents don't have ChipAuthInfo items.
    ...
    private func doCA( keyId: Int?, encryptionDetailsOID oid: String, publicKey: OpaquePointer) async throws {
        ...
        // Send the public key to the document
        try await sendPublicKey(oid: oid, keyId: keyId, pcdPublicKey: ephemeralKeyPair!)
            
        Log.debug( "Public Key successfully sent to document!" )
        
        // Use our ephemeral private key and the documents public key to generate a shared secret
        // (the document with do the same thing with their private key and our public key)
    ...
    private func sendPublicKey(oid : String, keyId : Int?, pcdPublicKey : OpaquePointer) async throws {
        ...
        guard let keyData = OpenSSLUtils.getPublicKeyData(from: pcdPublicKey) else {
            throw NFCDocumentReaderError.InvalidDataPassed("Unable to get public key data from public key" )
        }
        ...
        if cipherAlg.hasPrefix("DESede") {
            ...
            throw NFCDocumentReaderError.InvalidDataPassed("Cipher Algorithm \(cipherAlg) not supported")
        }
    }
    ...
    private func restartSecureMessaging( oid : String, sharedSecret : [UInt8], maxTranceiveLength : Int, shouldCheckMAC : Bool) throws  {
        ...
        if (cipherAlg.hasPrefix("DESede")) {
            ...
            throw NFCDocumentReaderError.InvalidDataPassed("Unsupported cipher algorithm \(cipherAlg)" )
        }
    }
    ...
    func inferDigestAlgorithmFromCipherAlgorithmForKeyDerivation( cipherAlg : String, keyLength : Int) throws -> String {
        ...
        throw NFCDocumentReaderError.InvalidDataPassed("Unsupported cipher algorithm or key length")
    }
