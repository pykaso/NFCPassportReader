...
class SecureMessagingSessionKeyGenerator {
    ...
    func deriveKey(keySeed: [UInt8], cipherAlgName: String, keyLength: Int, nonce: [UInt8]?, mode: SMSMode, paceKeyReference: UInt8) throws -> [UInt8] {
        ...
        if cipherAlgName == "DESede" || cipherAlgName == "3DES" {
            ...
            case 112, 128:
                ...
                break
            default:
                throw NFCDocumentReaderError.InvalidDataPassed("Can only use DESede with 128-but key length")
            }
            ...
            default:
                throw NFCDocumentReaderError.InvalidDataPassed("Can only use AES with 128-bit, 192-bit key or 256-bit length")
            }
        } else {
            throw NFCDocumentReaderError.InvalidDataPassed("Unsupported cipher algorithm used")
        }
    ...
    func getHash(algo: String, dataElements: [Data]) throws -> [UInt8] {
        ...
        if algo == "sha1" {
            ...
            throw NFCDocumentReaderError.InvalidHashAlgorithmSpecified
        }
