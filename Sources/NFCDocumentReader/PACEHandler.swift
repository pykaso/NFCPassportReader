...
public class PACEHandler {
    ...
    public init(cardAccess : CardAccess, tagReader: TagReader) throws {
        ...
        guard let pi = cardAccess.paceInfo else {
            throw NFCDocumentReaderError.NotYetSupported( "PACE not supported" )
        }
    ...
    public func doPACE( mrzKey : String ) async throws {
        ...
        let ephemeralParams = try await self.doStep2(documentNonce: decryptedNonce)
        let (ephemeralKeyPair, documentPublicKey) = try await self.doStep3KeyExchange(ephemeralParams: ephemeralParams)
        let (encKey, macKey) = try await self.doStep4KeyAgreement( pcdKeyPair: ephemeralKeyPair, documentPublicKey: documentPublicKey)
...
/*
        if needToTerminateGA {
            // This is to fix some documents that don't automatically terminate command chaining!
...
*/
    ...
    /// Performs PACE Step 1- receives an encrypted nonce from the document and decypts it with the  PACE key - derived from MRZ, CAN (not yet supported)
    func doStep1() async throws -> [UInt8] {
        ...
        if self.cipherAlg == "DESede" {
            ...
            throw NFCDocumentReaderError.UnsupportedCipherAlgorithm
        }
    ...
    /// Performs PACE Step 2 - computes ephemeral parameters by mapping the nonce received from the document
    ///  (and if IM used the nonce generated by us)
    ///
    /// Using the supported
    /// - Parameters:
    ///   - documentNonce: The decrypted nonce received from the document
    func doStep2( documentNonce: [UInt8]) async throws -> OpaquePointer {
        Log.debug( "Doing PACE Step2...")
        switch(mappingType) {
            case .CAM, .GM:
                Log.debug( "   Using General Mapping (GM)...")
                return try await doPACEStep2GM(documentNonce: documentNonce)
            case .IM:
                Log.debug( "   Using Integrated Mapping (IM)...")
                return try await doPACEStep2IM(documentNonce: documentNonce)
            default:
                throw NFCDocumentReaderError.PACEError( "Step2GM", "Unsupported Mapping Type" )
        }
    ...
    /// - Parameters:
    ///   - documentNonce: The decrypted nonce received from the document
    func doPACEStep2GM(documentNonce : [UInt8]) async throws -> OpaquePointer {
        ...
        guard let pcdMappingEncodedPublicKey = OpenSSLUtils.getPublicKeyData(from: mappingKey) else {
            throw NFCDocumentReaderError.PACEError( "Step2GM", "Unable to get public key from mapping key")
        }
        Log.verbose( "public mapping key - \(binToHexRep(pcdMappingEncodedPublicKey, asArray:true))")

        Log.debug( "Sending public mapping key to document..")
        ...
        Log.debug( "Received documents public mapping key")
        ...
        guard let bn_nonce = BN_bin2bn(documentNonce, Int32(documentNonce.count), nil) else {
            throw NFCDocumentReaderError.PACEError( "Step2GM", "Unable to convert picc nonce to bignum" )
        }
        ...
        if self.agreementAlg == "DH" {
            ...
            ephemeralParams = try self.doDHMappingAgreement(mappingKey: mappingKey, documentPublicKeyData: piccMappingEncodedPublicKey, nonce: bn_nonce )
        } else if self.agreementAlg == "ECDH" {
            Log.debug( "Doing ECDH Mapping agreement")
            ephemeralParams = try self.doECDHMappingAgreement(mappingKey: mappingKey, documentPublicKeyData: piccMappingEncodedPublicKey, nonce: bn_nonce )
        } else {
            throw NFCDocumentReaderError.PACEError( "Step2GM", "Unsupported agreement algorithm" )
        }
    ...
    func doPACEStep2IM( documentNonce: [UInt8] ) async throws -> OpaquePointer {
        // Not implemented yet
        throw NFCDocumentReaderError.PACEError( "Step2IM", "IM not yet implemented" )
    }
    
    /// Generates an ephemeral public/private key pair based on mapping parameters from step 2, and then sends
    /// the public key to the document and receives its ephmeral public key in exchange
    /// - Parameters:
    ///     - ephemeralParams: The ehpemeral mapping keys generated by step2
    /// - Returns:
///         - Tuple of Generated Ephemeral KeyPair and the document's public key
    func doStep3KeyExchange(ephemeralParams: OpaquePointer) async throws -> (OpaquePointer, OpaquePointer) {
        ...
        guard let ephemeralKeyPair = ephKeyPair else {
            throw NFCDocumentReaderError.PACEError( "Step3 KeyEx", "Unable to get create ephermeral key pair" )
        }
        ...
        guard let publicKey = OpenSSLUtils.getPublicKeyData( from: ephemeralKeyPair ) else {
            throw NFCDocumentReaderError.PACEError( "Step3 KeyEx", "Unable to get public key from ephermeral key pair" )
        }
        Log.verbose( "Ephemeral public key - \(binToHexRep(publicKey, asArray: true))")

        // exchange public keys
        Log.debug( "Sending ephemeral public key to document")
        ...
        let documentEncodedPublicKey = try? unwrapDO(tag: 0x84, wrappedData: response.data)
        guard let documentPublicKey = OpenSSLUtils.decodePublicKeyFromBytes(pubKeyData: documentEncodedPublicKey!, params: ephemeralKeyPair) else {
            throw NFCDocumentReaderError.PACEError( "Step3 KeyEx", "Unable to decode documents ephemeral key" )
        }

        Log.verbose( "Received documents ephemeral public key - \(binToHexRep(documentEncodedPublicKey!, asArray: true))" )
        return (ephemeralKeyPair, documentPublicKey)
    }
    
    /// This performs PACE Step 4 - Key Agreement.
    /// Here the shared secret is computed from our ephemeral private key and the documents ephemeral public key
    /// The new secure messaging (ksEnc and ksMac) keys are computed from the shared secret
    /// An authentication token is generated from the documents public key and the computed ksMac key
    /// Then, the authetication token is send to the document, it returns its own computed authentication token
    ...
    ///     - documentPublicKey: documents ephemeral public key
    /// - Returns:
    ///         - Tuple of KSEnc KSMac
    func doStep4KeyAgreement( pcdKeyPair: OpaquePointer, documentPublicKey: OpaquePointer) async throws -> ([UInt8], [UInt8]) {
        Log.debug( "Doing PACE Step4 Key Agreement...")

        Log.debug( "Computing shared secret...")
        let sharedSecret = OpenSSLUtils.computeSharedSecret(privateKeyPair: pcdKeyPair, publicKey: documentPublicKey)
        ...
        guard let pcdAuthToken = try? generateAuthenticationToken( publicKey: documentPublicKey, macKey: macKey) else {
            throw NFCDocumentReaderError.PACEError( "Step3 KeyAgreement", "Unable to generate authentication token using documents public key" )
        }
        Log.verbose( "authentication token - \(pcdAuthToken)")

        Log.debug( "Sending auth token to document")
        ...
        Log.verbose( "Expecting authentication token from document - \(expectedPICCToken)")

        let piccToken = [UInt8](tvlResp[0].value)
        Log.verbose( "Received authentication token from document - \(piccToken)")

        guard piccToken == expectedPICCToken else {
            Log.error( "Error PICC Token mismatch!\npicToken - \(piccToken)\nexpectedPICCToken - \(expectedPICCToken)" )
            throw NFCDocumentReaderError.PACEError( "Step3 KeyAgreement", "Error PICC Token mismatch!\npicToken - \(piccToken)\nexpectedPICCToken - \(expectedPICCToken)" )
        }
        
        Log.debug( "Auth token from document matches expected token!" )
    ...
    func paceCompleted( ksEnc: [UInt8], ksMac: [UInt8] ) throws {
        ...
        if (cipherAlg.hasPrefix("DESede")) {
            ...
            throw NFCDocumentReaderError.PACEError( "PACECompleted", "Not restarting secure messaging as unsupported cipher algorithm requested - \(cipherAlg)" )
        }
    }
}
...
extension PACEHandler {
    ...
    /// - Parameter documentPublicKeyData - byte array containing the publick key read from the document
    ...
    func doDHMappingAgreement( mappingKey : OpaquePointer, documentPublicKeyData: [UInt8], nonce: OpaquePointer ) throws -> OpaquePointer {
        ...
        // Compute the shared secret using the mapping key and the documents public mapping key
        let bn = BN_bin2bn(documentPublicKeyData, Int32(documentPublicKeyData.count), nil)
        defer { BN_free( bn ) }
    ...
    /// - Parameter nonce - Pointer to an BIGNUM structure containing the unencrypted nonce
    /// - Returns the EVP_PKEY containing the mapped ephemeral parameters
    func doECDHMappingAgreement( mappingKey : OpaquePointer, documentPublicKeyData: [UInt8], nonce: OpaquePointer ) throws -> OpaquePointer {
        ...
        guard let sharedSecretMappingPoint = self.computeECDHMappingKeyPoint(privateKey: mappingKey, inputKey: documentPublicKeyData) else {
            // Error
            throw PACEHandlerError.ECDHKeyAgreementError( "Failed to compute new shared secret mapping point from mapping key and document public mapping key" )
        }
    ...
    func encodePublicKey( oid : String, key : OpaquePointer ) throws -> [UInt8] {
        ...
        guard let pubKeyData = OpenSSLUtils.getPublicKeyData(from: key) else {
            ...
            throw NFCDocumentReaderError.InvalidDataPassed("Unable to get public key data")
        }
        ...
        guard let encOid = TKBERTLVRecord(from: Data(encodedOid)) else {
            throw NFCDocumentReaderError.InvalidASN1Value
        }
    ...
    func computeECDHMappingKeyPoint( privateKey : OpaquePointer, inputKey : [UInt8] ) -> OpaquePointer? {
        ...
        // Multiply our private key with the documents public key to get a new point
