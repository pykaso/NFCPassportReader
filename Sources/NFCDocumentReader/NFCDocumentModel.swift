...
public class NFCDocumentModel {
    ...
    public private(set) lazy var documentType : String = { return String( documentDataElements?["5F03"]?.first ?? "?" ) }()
    public private(set) lazy var documentSubType : String = { return String( documentDataElements?["5F03"]?.last ?? "?" ) }()
    public private(set) lazy var documentNumber : String = { return (documentDataElements?["5A"] ?? "?").replacingOccurrences(of: "<", with: "" ) }()
    public private(set) lazy var issuingAuthority : String = { return documentDataElements?["5F28"] ?? "?" }()
    public private(set) lazy var documentExpiryDate : String = { return documentDataElements?["59"] ?? "?" }()
    public private(set) lazy var dateOfBirth : String = { return documentDataElements?["5F57"] ?? "?" }()
    public private(set) lazy var gender : String = { return documentDataElements?["5F35"] ?? "?" }()
    public private(set) lazy var nationality : String = { return documentDataElements?["5F2C"] ?? "?" }()
    ...
    public private(set) lazy var documentMRZ : String = { return documentDataElements?["5F1F"] ?? "NOT FOUND" }()
    
    // Extract fields from DG11 if present
    private lazy var names : [String] = {
        guard let dg11 = dataGroupsRead[.DG11] as? DataGroup11,
              let fullName = dg11.fullName?.components(separatedBy: "<<") else { return (documentDataElements?["5B"] ?? "?").components(separatedBy: "<<") }
        return fullName
    }()
    ...
    public private(set) lazy var personalNumber : String? = {
        ...
        return (documentDataElements?["53"] ?? "?").replacingOccurrences(of: "<", with: "" )
    }()
    ...
    public internal(set) var BACStatus : DocumentAuthenticationStatus = .notDone
    public internal(set) var PACEStatus : DocumentAuthenticationStatus = .notDone
    public internal(set) var chipAuthenticationStatus : DocumentAuthenticationStatus = .notDone

    public private(set) var documentCorrectlySigned : Bool = false
    public private(set) var documentSigningCertificateVerified : Bool = false
    public private(set) var documentDataNotTampered : Bool = false
    ...
    private var documentDataElements : [String:String]? {
    ...
    /// - Parameters:
    ///    selectedDataGroups - the Data Groups to be exported (if they are present in the document)
    ...
    public func dumpDocumentData( selectedDataGroups : [DataGroupId], includeActiveAuthenticationData : Bool = false) -> [String:String] {
    ...
    public func verifyDocument( masterListURL: URL?, useCMSVerification : Bool = false ) {
    ...
    // We do this by trying to build a trust chain of the document certificate against the ones in the revocation list
    ...
    private func validateAndExtractSigningCertificates( masterListURL: URL ) throws {
        self.documentCorrectlySigned = false
        ...
        Log.debug( "Document passed SOD Verification" )
        self.documentCorrectlySigned = true
    ...
    private func ensureReadDataNotBeenTamperedWith( useCMSVerification: Bool ) throws  {
        ...
        // Now Verify document data by comparing compare Hashes in SOD against
        // computed hashes to ensure data not been tampered with
        documentDataNotTampered = false
        ...
        Log.debug( "Document passed Datagroup Tampering check" )
        documentDataNotTampered = true
    }
